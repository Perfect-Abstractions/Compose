...
contract ERC20MissingExportSelector {
    ...
    /**
     * @notice Exports an intentionally incomplete selector list for validation testing.
     * @dev Missing this.decimals.selector on purpose.
     * @return selectors The exported function selectors.
     */
    function exportSelectors() external pure returns (bytes memory) {
        return bytes.concat(this.name.selector, this.symbol.selector);
    }
}
