...
contract MyERC721Facet {
    ...
    // Assume enumerableMod is initialized elsewhere with the correct storage pointer
    // For example, in an initializer function:
    // function initialize(address _enumerableModAddress) external initializer {
    //     enumerableMod = ERC721EnumerableMod(_enumerableModAddress);
    // }
    ...
    function burnToken(uint256 _tokenId, address _sender) external {
        ...
        enumerableMod.burn(_tokenId, _sender);
    }
    ...
    function transferToken(address _from, address _to, uint256 _tokenId, address _sender) external {
        ...
        enumerableMod.transferFrom(_from, _to, _tokenId, _sender);
    }

    function getEnumerableStorage() external view returns (ERC721EnumerableStorage memory) {
        // Retrieve the storage struct directly
        return enumerableMod.getStorage();
    }
}`}
</ExpandableCode>
-->
...
<LastUpdated date="2025-12-30T20:14:02.652Z" />
