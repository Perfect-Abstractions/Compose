...
- Exposes external view functions for ERC-721 data queries.
- Uses internal assembly to access diamond storage.
...
contract ERC721Consumer {
    ...
    // Example of how selectors can be exported
    function getERC721Selectors() public view returns (bytes memory) {
        IDiamond diamond = IDiamond(diamondAddress);
...
- Ensure this facet is correctly added to the diamond proxy during deployment.
- Call all facet functions through the diamond proxy address.
- Verify storage slot compatibility if upgrading or adding new facets.
</Callout>
...
<LastUpdated date="2026-03-12T19:44:35.466Z" />
