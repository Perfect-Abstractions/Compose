...
- Implements core ERC-6909 logic including mint, burn, transfer, and approval.
- Utilizes a dedicated storage slot for its state, aligning with the diamond storage pattern.
...
import {IERC6909Mod} from "@compose-protocol/diamond-contracts/contracts/modules/ERC6909/IERC6909Mod.sol";
import {ERC6909Mod} from "@compose-protocol/diamond-contracts/contracts/modules/ERC6909/ERC6909Mod.sol";

contract MyERC6909Facet {
    address immutable DIAMOND_ADDRESS;
    IERC6909Mod immutable erc6909Mod;

    constructor(address diamondAddress) {
        DIAMOND_ADDRESS = diamondAddress;
        erc6909Mod = IERC6909Mod(diamondAddress);
    }
...
- Ensure the `ERC6909Mod` is correctly initialized with the appropriate storage slot.
- Handle all custom errors defined by the module to prevent unexpected reverts.
- Be mindful of operator permissions when performing transfers on behalf of others.
</Callout>
...
<LastUpdated date="2025-12-30T14:41:46.829Z" />
