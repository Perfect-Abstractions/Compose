...
import { RoyaltyStorage } from "@compose/token/Royalty/RoyaltyMod";

contract RoyaltyFacet {
    // Assume RoyaltyMod is deployed and accessible
    // Assume STORAGE_POSITION is defined and known

    function _setTokenRoyalty(uint256 _tokenId, address _receiver, uint96 _feeNumerator) external {
        // Call internal function from RoyaltyMod
        RoyaltyMod.setTokenRoyalty(_tokenId, _receiver, uint256(_feeNumerator));
    }

    function _getDefaultRoyalty() external view returns (address receiver, uint256 feeNumerator) {
        RoyaltyStorage storage royaltyData = RoyaltyMod.getStorage();
        return (royaltyData.defaultRoyaltyInfo.receiver, royaltyData.defaultRoyaltyInfo.feeNumerator);
    }

    function _royaltyInfo(uint256 _tokenId, uint256 _salePrice) external view returns (address, uint256) {
        // Call internal function from RoyaltyMod
...
<LastUpdated date="2025-12-30T20:14:31.561Z" />
