...
description: "Mints ERC-1155 tokens and manages balances"
...
<DocSubtitle>
Mints ERC-1155 tokens and manages balances
</DocSubtitle>

<Callout type="info" title="Key Features">
- Internal functions for minting single and batched ERC-1155 tokens.
...
This module provides internal functions to mint ERC-1155 tokens and manage their balances within the diamond storage pattern. Facets can import this module to safely increase token supplies, emitting standard ERC-1155 transfer events. Receiver validation is performed for contract recipients to ensure compatibility.
...
contract MyERC1155Facet {
    ...
    function mintSingleToken(address _to, uint256 _id, uint256 _value, bytes _data) external {
        ERC1155Storage storage storage = ERC1155MintMod.getStorage();
        storage.mint(_to, _id, _value, _data);
    }

    function mintBatchTokens(address _to, uint256[] memory _ids, uint256[] memory _values, bytes _data) external {
        ERC1155Storage storage storage = ERC1155MintMod.getStorage();
        storage.mintBatch(_to, _ids, _values, _data);
    }
}`}
</ExpandableCode>
-->
...
- Ensure the `ERC1155Storage` struct is correctly initialized in diamond storage.
- Validate input array lengths for `mintBatch` to prevent `ERC1155InvalidArrayLength` errors.
- Handle `ERC1155InvalidReceiver` errors if minting to contract addresses.
</Callout>
...
<LastUpdated date="2026-03-12T19:43:58.832Z" />
