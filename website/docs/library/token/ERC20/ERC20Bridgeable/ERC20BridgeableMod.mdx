...
description: "Cross-chain token transfers with role-based access control"
...
<DocSubtitle>
Cross-chain token transfers with role-based access control
</DocSubtitle>

<Callout type="info" title="Key Features">
- Enables cross-chain minting and burning operations.
- Enforces `trusted-bridge` role for cross-chain actions.
- Uses diamond storage for shared state management.
- Functions are internal and designed for facet composition.
</Callout>
...
contract MyBridgeFacet {
    ...
    // Assume bridgeableMod is initialized with the correct diamond storage address
    constructor(address _diamondAddress) {
        bridgeableMod = ERC20BridgeableMod(_diamondAddress);
    }

    /**
     * @notice Mints tokens for a recipient on the current chain.
     * @param _account The address to mint tokens to.
     * @param _value The amount of tokens to mint.
     */
    function mintCrosschain(address _account, uint256 _value) external {
        // checkTokenBridge is implicitly called by crosschainMint
        bridgeableMod.crosschainMint(_account, _value);
    }

    /**
     * @notice Burns tokens from a sender on the current chain.
     * @param _from The address to burn tokens from.
     * @param _value The amount of tokens to burn.
     */
    function burnCrosschain(address _from, uint256 _value) external {
        // checkTokenBridge is implicitly called by crosschainBurn
...
- Ensure the `trusted-bridge` role is correctly managed via an access control module.
- Call `crosschainMint` and `crosschainBurn` only from authorized bridge contracts.
- Verify the `ERC20_STORAGE_POSITION` and `AccessControlStorage` struct compatibility during upgrades.
</Callout>

## Integration Notes

<Callout type="success" title="Shared Storage">
This module interacts with diamond storage at the `ERC20_STORAGE_POSITION`, identified by `keccak256("compose.erc20")`. It uses the `ERC20Storage` and `AccessControlStorage` structs. Functions like `crosschainMint` and `crosschainBurn` implicitly call `checkTokenBridge`, which verifies the caller's role against the access control storage. All state modifications are immediately visible to other facets accessing the same storage.
</Callout>
...
<LastUpdated date="2025-12-30T17:28:03.537Z" />
