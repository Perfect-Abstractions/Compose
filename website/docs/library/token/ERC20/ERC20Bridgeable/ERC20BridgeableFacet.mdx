...
- Self-contained facet with no external dependencies beyond diamond core.
</Callout>

## Overview

This facet enables cross-chain ERC-20 token minting and burning operations within a diamond. It routes calls through the diamond proxy and enforces access control for trusted bridge operators. Developers add this facet to integrate cross-chain token functionality while maintaining diamond upgradeability.
...
address diamondAddress = 0x...

function mintViaBridge(address _account, uint256 _value) {
    IDiamond(diamondAddress).crosschainMint(_account, _value);
}

function burnViaBridge(address _from, uint256 _value) {
    IDiamond(diamondAddress).crosschainBurn(_from, _value);
}

function checkBridgeStatus(address _caller) public view returns (bool) {
    // This function is called through the diamond proxy. The actual facet's checkTokenBridge is invoked.
    // The internal checkTokenBridge function within the facet will revert if the caller is not trusted.
    // For external visibility, a wrapper function might be needed or direct calls to the diamond proxy if exposed.
    // For demonstration, assume we are calling a public wrapper or directly through diamond:
    // NOTE: The provided contract data only shows 'checkTokenBridge' as external, not public.
    // If it's truly external, it can only be called by other facets or through the diamond proxy.
    // Assuming a scenario where it's callable via diamond proxy for this example:
    return IDiamond(diamondAddress).checkTokenBridge(_caller) == true; // This might not be the exact intended usage if checkTokenBridge is not public
}`}
</ExpandableCode>
-->
...
- Only call `crosschainMint` and `crosschainBurn` from authorized bridge contracts.
- Verify that the `ERC20BridgeableFacet` is correctly registered with the diamond proxy.
</Callout>

## Security Considerations

<Callout type="warning" title="Security">
Cross-chain operations are protected by role-based access control, requiring the `trusted-bridge` role for `crosschainMint` and `crosschainBurn` functions. The `checkTokenBridge` function validates the caller's role. Follow standard Solidity security practices for input validation and reentrancy.
</Callout>
...
<LastUpdated date="2025-12-30T18:50:24.026Z" />
