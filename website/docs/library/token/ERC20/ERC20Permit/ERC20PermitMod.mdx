...
- Implements ERC-2612 Permit logic for gasless approvals.
- Provides a standardized `DOMAIN_SEPARATOR` for signature validation.
- Includes a `permit` function to validate signatures and set allowances.
</Callout>
...
This module provides the core logic for implementing ERC-2612 Permit functionality, enabling gasless token approvals. It includes utilities for managing the domain separator and validating permit signatures, crucial for secure and efficient token transfers.
...
import {IERC20Permit} from "@openzeppelin/contracts/interfaces/draft-IERC677.sol";
...
contract MyERC20Facet {
    ...
    function permit(address owner, address spender, uint256 value, uint256 deadline, uint8 v, bytes32 r, bytes32 s)
        ...
        returns (bool)
    {
        // Ensure the calling contract's domain separator is correctly set
        bytes32 domainSeparator = ERC20PermitMod.DOMAIN_SEPARATOR();

        // Use the library function to validate and set allowance
        ...
        emit IERC20Permit.Approval(owner, spender, value);

        return true;
    }

    // Other ERC-20 functions would go here...
}`}
</ExpandableCode>
...
- Ensure the domain separator is correctly retrieved and used for signature validation.
- Always emit the `Approval` event after a successful `permit` call, as required by the ERC-2612 standard.
- Handle `ERC2612InvalidSignature` and `ERC20InvalidSpender` errors appropriately in your facet.
</Callout>

## Integration Notes

<Callout type="success" title="Shared Storage">
The `ERC20PermitMod` relies on specific storage slots within the diamond proxy for its `PermitStorage` and `ERC20Storage`. Facets integrating this module should ensure these storage layouts are correctly initialized and accessible. The `permit` function internally uses the `Approval` event, which must also be emitted by the calling facet to comply with ERC-20 standards.
</Callout>
...
<LastUpdated date="2025-12-28T04:47:35.330Z" />
