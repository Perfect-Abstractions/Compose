...
- Provides access to nonces and domain separator for signature verification.
- Utilizes diamond storage for nonce management.
- Self-contained facet with no external dependencies.
</Callout>

## Overview

This facet enables EIP-2612 permit functionality, allowing token owners to grant allowances via signed messages. It integrates with the diamond storage pattern to manage nonces and domain separators. Developers add this facet to a diamond to provide a gas-efficient way for users to approve token transfers without on-chain transactions.
...
contract MyDiamondUsers {
    ...
    function grantPermit(address _owner, address _spender, uint256 _value, uint256 _deadline, uint8 _v, bytes32 _r, bytes32 _s) external {
        IDiamond diamond = IDiamond(diamondAddress);
        // Call the permit function through the diamond proxy
        diamond.executeFacetFunction(
            ERC20PermitFacet.ERC20PERMIT_FACET_FUNCTION_SELECTORS.PERMIT,
            abi.encodeCall(ERC20PermitFacet.permit, (_owner, _spender, _value, _deadline, _v, _r, _s))
        );
    }

    function getNonces(address _owner) external view returns (uint256) {
        IDiamond diamond = IDiamond(diamondAddress);
        // Call the nonces function through the diamond proxy
        return abi.decode(diamond.executeFacetFunction(
            ERC20PermitFacet.ERC20PERMIT_FACET_FUNCTION_SELECTORS.NONCES,
            abi.encodeCall(ERC20PermitFacet.nonces, (_owner))
        ), (uint256));
    }
    ...
    function getDomainSeparator() external view returns (bytes32) {
        ...
        return abi.decode(diamond.executeFacetFunction(
            ERC20PermitFacet.ERC20PERMIT_FACET_FUNCTION_SELECTORS.DOMAIN_SEPARATOR,
            ''
        ), (bytes32));
    }
}`}
...
- Initialize the ERC20PermitStorage struct when deploying the diamond.
- Ensure the DOMAIN_SEPARATOR is correctly configured for replay protection.
- Validate the signature parameters (_v, _r, _s) before calling the permit function.
</Callout>
...
<LastUpdated date="2025-12-30T17:27:45.564Z" />
