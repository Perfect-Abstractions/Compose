...
- Implements EIP-2612 permit functionality for ERC-20 tokens.
- Exposes `permit`, `nonces`, and `DOMAIN_SEPARATOR` functions.
...
contract ExampleUsage {
    ...
    function examplePermit() public {
        IDiamond diamond = IDiamond(diamondAddress);
        // Example: Owner grants allowance to spender via permit
        // This call would typically be made by the owner or a delegated entity
        // after obtaining a valid signature.
        // The actual signature parameters (_v, _r, _s) are generated off-chain.
        address owner = msg.sender; // Or any other owner address
        address spender = address(1);
        uint256 value = 1000 ether;
        uint256 deadline = block.timestamp + 1 days;
        uint8 v = 1; // Placeholder for actual v value
        bytes32 r = "0x0"; // Placeholder for actual r value
        bytes32 s = "0x0"; // Placeholder for actual s value

        diamond.executeFacetFunction(
            ERC20PermitFacet.FACET_FUNCTION_SELECTORS.permit,
            abi.encodeCall(
    ...
    function getDomainSeparator() public view returns (bytes32) {
        IDiamond diamond = IDiamond(diamondAddress);
        return diamond.executeFacetFunction(ERC20PermitFacet.FACET_FUNCTION_SELECTORS.DOMAIN_SEPARATOR, "");
    }

    function getNonces(address _owner) public view returns (uint256) {
        IDiamond diamond = IDiamond(diamondAddress);
        return diamond.executeFacetFunction(ERC20PermitFacet.FACET_FUNCTION_SELECTORS.nonces, abi.encode(_owner));
    }
}`}
</ExpandableCode>
-->
...
- Initialize the ERC20PermitFacet with necessary state during diamond deployment.
- Ensure that signature verification for the `permit` function is handled correctly off-chain before submitting to the diamond.
- Verify that the DOMAIN_SEPARATOR returned by the facet is correctly used in signature generation to prevent replay attacks across different diamonds or chains.
</Callout>
...
<LastUpdated date="2025-12-30T18:50:04.310Z" />
