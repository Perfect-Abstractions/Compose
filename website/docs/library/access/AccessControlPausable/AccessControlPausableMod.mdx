...
description: "Manage paused roles using diamond storage"
...
contract MyAccessFacet {
    ...
    // For example, via a diamond initializer or a shared registry.

    function grantRoleAndPause(bytes32 _role, address _account) external {
        // Assume grantRole logic exists and is called before this.
        // Call internal function to pause the role.
        accessControlPausableMod.pauseRole(_role);
    }

    function unpauseRoleAndCheck(bytes32 _role, address _account) external {
        // Call internal function to unpause the role.
        accessControlPausableMod.unpauseRole(_role);
        // Check if the account has the role and if it's not paused.
...
<LastUpdated date="2025-12-30T20:12:45.818Z" />
