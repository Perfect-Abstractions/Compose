...
description: "Role and pause management for diamond facets"
...
<DocSubtitle>
Role and pause management for diamond facets
</DocSubtitle>

<Callout type="info" title="Key Features">
- Provides internal functions for pausing and checking role states.
- Integrates with diamond storage pattern using `ACCESS_CONTROL_STORAGE_POSITION`.
- Reverts with specific errors for paused roles or unauthorized accounts.
...
This module provides internal functions to manage role pausing and checking within a diamond. Facets can import this module to enforce role-specific pause states before executing sensitive operations. This ensures that paused roles prevent unauthorized actions, enhancing diamond security and upgradeability.
...
contract MyAccessFacet {
    ...
    function initialize(AccessControlPausableMod _accessControlPausableMod) external {
        accessControlPausableMod = _accessControlPausableMod;
    }

    bytes32 public constant MY_ROLE = keccak256("MY_ROLE");

    function executeActionForRole() external {
        address caller = msg.sender;
        // Ensure role is not paused before proceeding
        accessControlPausableMod.requireRoleNotPaused(MY_ROLE, caller);

        // ... execute action ...
    }

    function pauseMyRole() external {
        // Only an authorized entity can pause the role
        // Assuming OwnerTwoStepsMod or similar handles authorization for pausing
        accessControlPausableMod.pauseRole(MY_ROLE);
    }

    function unpauseMyRole() external {
        // Assuming OwnerTwoStepsMod or similar handles authorization for unpausing
...
- Call `requireRoleNotPaused` before executing role-dependent logic to prevent actions when a role is paused.
- Ensure access control for pausing and unpausing roles is handled by a separate, authorized module (e.g., `OwnerTwoStepsMod`).
- Handle `AccessControlRolePaused` and `AccessControlUnauthorizedAccount` errors explicitly in calling facets.
</Callout>

## Integration Notes

<Callout type="success" title="Shared Storage">
This module stores its state within the diamond's shared storage at the `ACCESS_CONTROL_STORAGE_POSITION`, identified by `keccak256("compose.accesscontrol")`. All functions interact with the `AccessControlPausableStorage` struct. Changes to role pause states made through this module are immediately visible to all facets that access the same storage slot.
</Callout>
...
<LastUpdated date="2025-12-30T18:49:25.476Z" />
