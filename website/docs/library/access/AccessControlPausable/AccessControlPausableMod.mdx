...
description: "Manage role-based access control and pausing."
...
<DocSubtitle>
Manage role-based access control and pausing.
</DocSubtitle>

<Callout type="info" title="Key Features">
- Role-specific pausing and unpausing of permissions.
- Reverts with specific errors (`AccessControlUnauthorizedAccount`, `AccessControlRolePaused`) for clear failure analysis.
...
This module provides robust role-based access control combined with the ability to pause specific roles. It ensures that sensitive functions can be temporarily halted for specific roles without affecting others, enhancing operational safety and control within a Compose diamond.
...
contract MyFacet {
    ...
    function _setupRole(bytes32 role, address account) internal {
        // Assuming role setup is handled by another facet or initialization logic
        // This is just an example of how to interact with the module
    }

    function _pauseRole(bytes32 role) external {
        accessControlPausableMod.pauseRole(role);
    }

    function _unpauseRole(bytes32 role) external {
        accessControlPausableMod.unpauseRole(role);
    }

    function _isRolePaused(bytes32 role) external view returns (bool) {
        return accessControlPausableMod.isRolePaused(role);
    }

    function _requireRoleNotPaused(bytes32 role, address account) internal {
        accessControlPausableMod.requireRoleNotPaused(role, account);
    }
}`}
</ExpandableCode>
...
- Use `requireRoleNotPaused` to enforce role presence and ensure the role is not currently paused before executing sensitive operations.
- Implement pause/unpause functionality judiciously, only when necessary for operational safety, and ensure clear communication to users when roles are paused.
- Integrate with the diamond's upgradeability pattern to ensure the AccessControlPausableMod is correctly updated if its implementation changes.
</Callout>

## Integration Notes

<Callout type="success" title="Shared Storage">
The AccessControlPausableMod manages its state within its own storage slots. Facets interacting with this module should obtain a reference to its implementation contract. The `requireRoleNotPaused` function checks both role membership (delegated to underlying access control) and the pause status managed by this module. Any changes to the pause status are immediately reflected upon calling `pauseRole` or `unpauseRole`.
</Callout>
...
<LastUpdated date="2025-12-28T04:27:47.507Z" />
