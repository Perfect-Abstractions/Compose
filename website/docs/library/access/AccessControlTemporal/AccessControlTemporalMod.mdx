...
description: "Manages time-bound role assignments in a diamond"
...
contract MyAccessFacet {
    ...
    constructor(address diamondAddress) {
        // Assuming AccessControlTemporalMod is deployed at the diamondAddress or has a known address
        acTemporal = AccessControlTemporalMod(diamondAddress);
    }

    /**
     * @notice Grants a role with an expiry.
     */
    function grantTempRole(bytes32 _role, address _account, uint256 _expiresAt) external {
        // Call the module's function to grant the role with expiry
        bool success = acTemporal.grantRoleWithExpiry(_role, _account, _expiresAt);
        require(success, "Failed to grant temporal role");
    }

    /**
     * @notice Checks if a role is currently valid and not expired.
     */
    function checkRoleValidity(bytes32 _role, address _account) external view {
        // Use the module's validation function
        acTemporal.requireValidRole(_role, _account);
    }

    /**
     * @notice Revokes a time-bound role.
     */
    function revokeTempRole(bytes32 _role, address _account) external {
        // Call the module's function to revoke the role
        bool success = acTemporal.revokeTemporalRole(_role, _account);
        require(success, "Failed to revoke temporal role");
    }
}`}
</ExpandableCode>
-->
...
<LastUpdated date="2025-12-30T20:12:57.720Z" />
