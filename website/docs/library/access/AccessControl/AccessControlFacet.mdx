...
contract ExampleDiamond {
    ...
    function grantAdminRole(address account) public {
        ...
        IDiamond(DIAMOND_ADDRESS).grantRole(ADMIN_ROLE, account);
    }

    function checkMinterRole(address account) public view returns (bool) {
        // Calls are routed through the diamond proxy
        return IDiamond(DIAMOND_ADDRESS).hasRole(MINTER_ROLE, account);
    }

    function setMinterRoleAdmin(bytes32 role) public {
        // Calls are routed through the diamond proxy
        IDiamond(DIAMOND_ADDRESS).setRoleAdmin(MINTER_ROLE, role);
    }
}`}
</ExpandableCode>
-->
...
<LastUpdated date="2025-12-30T20:13:14.399Z" />
