...
- Reverts if no facet is registered for the selector (see [`FunctionNotFound`](#errors) error).
...
<Callout type="warning" title="Execution Context">
  ...
  properties={[
    {
      ...
      descriptionElement: (
        <>
          The facet addresses to add. Each must implement{" "}
          <a href="https://github.com/Perfect-Abstractions/Compose/blob/main/src/interfaces/IFacet.sol"><code>IFacet</code></a>
          {" "}and return selectors from <code>exportSelectors()</code>.
        </>
      )
    }
  ]}
...
<b>Used internally by [`addFacets`](#addfacets).</b>
...
<AccordionGroup>
  <Accordion title="CannotAddFunctionToDiamondThatAlreadyExists" defaultOpen={false}>
    <div style={{marginBottom: "1rem"}}>
      Thrown by [<code>addFacets</code>](#addfacets) when a selector from one of the facets is already registered in the diamond via another facet.
    </div>
    <div>
      <strong>Signature:</strong>
      <ExpandableCode language="solidity" maxLines={5}>
...
- Verify that the `DiamondStorage` struct is correctly defined and <b>that any new fields are added at the end to maintain storage layout compatibility</b>.
...
<LastUpdated date="2026-05-01T12:49:43.377Z" />
