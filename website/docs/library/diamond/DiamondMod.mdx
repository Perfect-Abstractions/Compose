...
Registers one or more facets to a diamond. For each facet, selectors are discovered by calling `exportSelectors()`; each selector is then mapped to that facet in diamond storage. Reverts if any selector is already registered on the diamond.
