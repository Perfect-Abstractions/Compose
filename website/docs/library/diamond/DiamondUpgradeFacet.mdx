...
<Callout type="important" title="Facet Interface Requirement">
Facets must implement `exportSelectors()` so the upgrade logic can discover which selectors they handle.
...
Only the exported selector are discoverable to diamonds.
</Callout>
...
It is protected by [Owner](/docs/library/access/Owner/) checks, and it optionally executes an unrestricted `delegatecall` after facet updates. Treat `_delegate` and `_delegateCalldata` as fully trusted: that code can modify diamond storage and may introduce complex control flow.

<div style={{marginTop: "3rem", paddingTop: "2rem", borderTop: "1px solid var(--ifm-color-emphasis-200)"}}>
  <WasThisHelpful pageId="Diamond Upgrade Facet" />
</div>

<LastUpdated date="2026-03-13T01:49:50.563Z" />
