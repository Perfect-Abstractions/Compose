...
- Returns unique facet addresses and their associated function selectors.
- Optimized for efficient querying even with a large number of facets and selectors.
</Callout>
...
contract MyDiamond {
    ...
    constructor(address _diamondLoupeFacetAddress) {
        diamondLoupeFacet = IDiamondLoupe(_diamondLoupeFacetAddress);
    }
...
- Use `facets()` to get a comprehensive view of all registered facets and their selectors.
...
This facet is read-only and does not modify state, posing minimal security risks. Ensure the facet address provided during deployment is trusted.
</Callout>
...
<LastUpdated date="2025-12-28T04:49:19.806Z" />
