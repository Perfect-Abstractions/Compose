...
description: "Upgrade diamond with new, replaced, and removed facets"
...
<DocSubtitle>
Upgrade diamond with new, replaced, and removed facets
</DocSubtitle>

<Callout type="info" title="Key Features">
- Manages diamond upgrades by adding, replacing, and removing functions.
- Emits detailed events for all upgrade operations (`DiamondFunctionAdded`, `DiamondFunctionReplaced`, `DiamondFunctionRemoved`).
- Supports optional delegate calls for state modifications during upgrades.
- No external dependencies, designed for direct integration.
</Callout>
...
contract ExampleFacet {
    ...
    function upgradeMyFacet() external {
        ...
        functionsToAdd[0].functionSelectors[0] = this.upgradeMyFacet.selector;

        diamondUpgradeMod.upgradeDiamond(
            functionsToAdd,
            new FacetFunctions[](0), // No replacements
            selectorsToRemove,
...
- Ensure `_tag` and `_metadata` are used for audit trails when applicable.
- Handle potential `DelegateCallReverted` errors if a delegate call is performed.
- Use explicit checks for existing selectors before adding or replacing functions to prevent `CannotAddFunctionToDiamondThatAlreadyExists`.
</Callout>
...
<LastUpdated date="2025-12-30T18:51:35.955Z" />
