...
contract MyFacet {
    DiamondUpgradeMod immutable diamondUpgradeMod;

    constructor(address diamondUpgradeModAddress) {
        diamondUpgradeMod = DiamondUpgradeMod(diamondUpgradeModAddress);
    }

    function upgradeMyDiamond() external {
        FacetFunctions[] memory functionsToAdd = new FacetFunctions[](1);
        functionsToAdd[0] = FacetFunctions({
            facetAddress: address(this),
            selectors: new bytes4[](1)
        });
        functionsToAdd[0].selectors[0] = bytes4(keccak256("myNewFunction()"));

        bytes4[] memory functionsToRemove = new bytes4[](1);
        functionsToRemove[0] = bytes4(keccak256("oldFunction()    ")); // Note: 4 bytes padding for selector

        diamondUpgradeMod.upgradeDiamond(
            functionsToAdd,
            new FacetFunctions[](0), // No functions to replace
            functionsToRemove,
...
<LastUpdated date="2025-12-30T20:14:56.212Z" />
