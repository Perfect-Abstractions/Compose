...
import {
    ERC721EnumerableTransferMod_Base_Test
} from "test/unit/token/ERC721/Enumerable/ERC721EnumerableTransferModBase.t.sol";
import {ERC721StorageUtils} from "test/utils/storage/ERC721StorageUtils.sol";

import "src/token/ERC721/Enumerable/Transfer/ERC721EnumerableTransferMod.sol";

/**
 *  @dev BTT spec: test/trees/ERC721.tree
 */
contract Transfer_ERC721EnumerableTransferMod_Fuzz_Unit_Test is ERC721EnumerableTransferMod_Base_Test {
    ...
    function testFuzz_ShouldUpdateEnumerationOnTransfer_WhenCalledWithCorrectOwner(
        address owner,
        address to,
        uint256 tokenId
    ) external {
        ...
        assertEq(address(harness).balanceOf(owner), 0, "old owner balance");
        assertEq(address(harness).balanceOf(to), 1, "new owner balance");
    }
}
