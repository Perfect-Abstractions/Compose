 ...
 */
contract GetApproved_ERC721DataFacet_Fuzz_Unit_Test is ERC721DataFacet_Base_Test {
    ...
    function testFuzz_GetApproved(address owner, address approved, uint256 tokenId) external whenTokenExists {
        vm.assume(owner != ADDRESS_ZERO);

        address(facet).setOwnerOf(tokenId, owner);
        address(facet).setApproved(tokenId, approved);

        assertEq(facet.getApproved(tokenId), approved, "getApproved(tokenId)");
    }
}
